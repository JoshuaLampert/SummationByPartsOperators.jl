...
if VERSION >= v"1.9"
  @testset "Function space operators" begin
    ...
    for consistency_constraint in [false, true]
      N = 5
      x_min = -1.0
      x_max = 1.0
      ...
      let basis_functions = [x -> x^i for i in 0:3]
        D = function_space_operator(basis_functions, nodes, source, consistency_constraint = consistency_constraint)
    ...
    let basis_functions = [identity, exp]
      D = function_space_operator(basis_functions, nodes, source, consistency_constraint = true)

      @test grid(D) ≈ nodes
      @test all(isapprox.(D * ones(N), zeros(N); atol = 1e-15))
      @test D * nodes ≈ ones(N)
      @test D * exp.(nodes) ≈ exp.(nodes)
      M = mass_matrix(D)
      @test M * D.D + D.D' * M ≈ B
